...
/* OpenHMD dumper Driver - HID/USB Driver Implementation */
...
typedef struct {
	...
	dumper_coordinate_frame coordinate_frame, hw_coordinate_frame;
...
static dumper_priv* dumper_priv_get(ohmd_device* device)
{
	return (dumper_priv*)device;
}

static int get_feature_report(dumper_priv* priv, dumper_sensor_feature_cmd cmd, unsigned char* buf)
{
...
static int send_feature_report(dumper_priv* priv, const unsigned char *data, size_t length)
{
	return hid_send_feature_report(priv->handle, data, length);
}

static void set_coordinate_frame(dumper_priv* priv, dumper_coordinate_frame coordframe)
{
	priv->coordinate_frame = coordframe;

	// set the dumper_SCF_SENSOR_COORDINATES in the sensor config to match whether coordframe is hmd or sensor
	SETFLAG(priv->sensor_config.flags, dumper_SCF_SENSOR_COORDINATES, coordframe == dumper_CF_SENSOR);

	// encode send the new config to the dumper
	...
	if(size <= 0){
		...
		priv->hw_coordinate_frame = dumper_CF_HMD;
		return;
	}

	decode_sensor_config(&priv->sensor_config, buf, size);
	priv->hw_coordinate_frame = (priv->sensor_config.flags & dumper_SCF_SENSOR_COORDINATES) ? dumper_CF_SENSOR : dumper_CF_HMD;
...
static void handle_tracker_sensor_msg(dumper_priv* priv, unsigned char* buffer, int size)
{
	...
	if (buffer[0] == dumper_IRQ_SENSORS_DK2
	...
	vec3f_from_dumper_vec(mag32, &priv->raw_mag);

	for(int i = 0; i < OHMD_MIN(s->num_samples, 3); i++){
		vec3f_from_dumper_vec(s->samples[i].accel, &priv->raw_accel);
		vec3f_from_dumper_vec(s->samples[i].gyro, &priv->raw_gyro);
...
{
	dumper_priv* priv = dumper_priv_get(device);
	...
	while(true){
		...
		if(buffer[0] == dumper_IRQ_SENSORS || buffer[0] == dumper_IRQ_SENSORS_DK2) {
...
{
	dumper_priv* priv = ohmd_alloc(driver->ctx, sizeof(dumper_priv));
	if(!priv)
		goto cleanup;
	...
	size = get_feature_report(priv, dumper_CMD_RANGE, buf);
	...
	priv->coordinate_frame = priv->display_info.distortion_type != dumper_DT_NONE ? dumper_CF_HMD : dumper_CF_SENSOR;

	// enable calibration
	SETFLAG(priv->sensor_config.flags, dumper_SCF_USE_CALIBRATION, 1);
	SETFLAG(priv->sensor_config.flags, dumper_SCF_AUTO_CALIBRATION, 1);
...
ohmd_driver* ohmd_create_dumper_drv(ohmd_context* ctx)
{
